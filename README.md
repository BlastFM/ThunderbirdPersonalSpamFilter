...
* Corrected the README release history so the current v1.4.39 ruleset and v1.4.38 classifier-context changes are documented alongside the matching download links and package version.

### [v1.4.39] - 2026-09-09 (Stable)

* Replaced the downloadable Custom Classification Prompt Rules with the compact `EMAIL SPAM CLASSIFIER - V2 COMPACT PRODUCTION` ruleset, with a clearer decision order, tighter decisive/strong/weak evidence hierarchy, and the same conservative HAM default.
* Preserved and documented the add-on's hard local FQDN validation rule: malformed or non-public sender addresses and present malformed Reply-To addresses are routed to SPAM before AI classification; a missing Reply-To remains allowed.
* Added policy JSON regression coverage to ensure the downloadable rules import as Custom Classification Prompt Rules and retain the FQDN guidance.

### [v1.4.38] - 2026-09-09 (Stable)

* Expanded the evidence sent to OpenAI so Custom Classification Prompt Rules can evaluate the context they reference: relevant headers, Reply-To, authentication results, attachment metadata, and up to 6,000 body characters.
* Added regression coverage proving custom rules are included in the OpenAI system prompt and that content beyond the old 1,500-character limit reaches classification.

### [v1.4.37] - 2026-09-09 (Stable)

* Updated the downloadable Conservative Classification Policy to the new V2 production rules, adding stronger guidance against prompt-injection attempts inside email content, clearer HAM defaults for legitimate noreply/automated/third-party mail, and more detailed phishing, fraud, malware, impersonation, URL, QR-code, attachment, financial-scam, and social-engineering handling.
* Preserved the add-on's hard local FQDN rule in the policy: absent/unusable/malformed sender addresses and present malformed Reply-To addresses are SPAM, while a missing Reply-To remains normal and is not suspicious by itself.
* Packaged the new policy in the v1.4.37 XPI and kept the README download links pointed at the matching release assets.
...
* Restore destination now uses the message's **To:** address to pick the right account when no per-message origin folder is recorded. The To: address identifies which account received the email, so it is matched against each account's identity email and the message is restored to that account's Inbox — fixing restores landing in the wrong account's Inbox for legacy log entries. Falls back to the most common logged origin folder, then the default Inbox, only when no recipient matches a known account.

### [v1.4.34] - 2026-09-08 (Stable)

* Smarter restore destination when a spam log entry has no recorded origin folder (entries written by older versions, or messages that were already in the spam destination when logged). Instead of always falling back to the profile's default account Inbox — which sent restores to the wrong folder for anyone filtering from a different account — the restore now reuses the most common origin folder recorded by your other recent log entries first, and only uses the generic Inbox as a last resort.

### [v1.4.33] - 2026-09-08 (Stable)

* Fixed "Mark as Not Spam (Train AI)" silently doing nothing when the same email exists in more than one folder (a duplicate delivery sharing one RFC822 Message-ID). The restore resolves the message via a Message-ID query that returns *all* copies, and previously took the first match — which could be a stale duplicate already sitting in the Inbox, turning the restore into a no-op move from Inbox to Inbox while the spam-folder copy you clicked was never touched. The resolver now prefers the exact message you acted on, and otherwise a copy that isn't in the spam destination.
* Spam log matching and the stored Message-IDs are now normalised for angle brackets (`<id@host>` vs `id@host`), so log entries written by older versions (which stored the ID bare) correctly match the clicked message again — restoring the per-message origin-folder lookup instead of falling back to the default Inbox.

### [v1.4.32] - 2026-09-08 (Stable)

* Added step-by-step diagnostic logging to the "Mark as Not Spam (Train AI)" restore path (visible in Thunderbird's Error Console, Ctrl+Shift+J): which message was resolved, which destination folder was chosen and why, whether the move/copy/delete steps reported success, and — for the copy+delete fallback — exactly which folders the verification query found copies in. This is a diagnostics-only release to pin down environment-specific restore failures; behaviour is unchanged from v1.4.31.

### [v1.4.31] - 2026-09-08 (Stable)

* Fixed "Mark as Not Spam (Train AI)" recording the AI Training Memory entry but leaving the message in the "AI Filtered Spam" folder. Thunderbird's `messages.move` API sits on the legacy `nsIMsgCopyService`, whose cross-account behaviour has been restricted since Thunderbird 91, so restoring out of the shared "Local Folders / AI Filtered Spam" folder back into a different account's Inbox could throw and leave the message stuck. The restore now falls back to the documented workaround: copy the message to the destination, verify the copy actually arrived, then delete the original (requires the new `messagesDelete` permission). If even the copy fails, the Detected Spam Log entry is rolled back so the message stays listed and the restore can be retried, and the failure is surfaced via the existing notification/status error instead of failing silently.

### [v1.4.30] - 2026-09-08 (Stable)

* Fixed a race where a message restored via "Mark as Not Spam (Train AI)" out of the shared "Local Folders / AI Filtered Spam" folder into a different account's Inbox could immediately bounce right back into the spam folder. Moving a message across accounts is a copy+delete internally and assigns it a new id, which can make Thunderbird surface the restored copy as "new mail" and re-trigger AI classification before the existing id-based de-duplication guard has a chance to catch it. `processIncomingMessages` now also hard-skips classification for any message that matches an entry in the AI Training Memory (Not Spam) list, and the training entry is now written to storage before the restore move (not after), closing the window where the override wasn't recorded yet.

### [v1.4.29] - 2026-09-08 (Stable)

* Fixed "Mark as Not Spam (Train AI)" not returning messages to their original folder when restoring from the shared "Local Folders / AI Filtered Spam" destination without a matching spam-log entry (e.g. after "Clear log", or once the 50-entry log history had aged the entry out). The fallback Inbox lookup was incorrectly searching the *current* folder's account — the shared Local Folders account, which normally has no Inbox — instead of the message's original account, so the restore silently failed and the message stayed in the spam folder. It now falls back to the profile's default account's Inbox, then the first other account with one.

### [v1.4.28] - 2026-09-07 (Stable)

* Added a Product Support contact link in the options-page footer and README for polite, direct support at `openai-spam-detector@blastfm.limited`.

### [v1.4.27] - 2026-09-07 (Stable)

* Resolved "Unsafe assignment to innerHTML" warnings flagged by the Thunderbird/Mozilla add-on validator by rendering log entries and the footer version text via safe DOM APIs (`createElement`/`textContent`/`append`) instead of `innerHTML` assignment. No functional change; the previously-used `escapeHtml()` helper is no longer needed and was removed.

### [v1.4.26] - 2026-09-07 (Stable)

* Changed the extension ID's domain from `blastfm.net` to `blastfm.limited` (`openai-spam-detector@blastfm.limited`) at the publisher's request, ahead of submission to addons.thunderbird.net. No functional change.

### [v1.4.25] - 2026-09-07 (Stable)

* Replaced the placeholder extension ID (`openai-spam-detector@example.com`) in `manifest.json` with `openai-spam-detector@blastfm.net` ahead of submission to addons.thunderbird.net, since Mozilla/Thunderbird's add-on validator rejects `example.com` and reserved domains as extension IDs. This has no effect on functionality; it only changes the internal identifier used to track the add-on across updates.

### [v1.4.24] - 2026-09-07 (Stable)

* Empty Detected Spam Log / AI Training Memory sections now show a subtle dashed-border placeholder box instead of plain floating text, making it visually clear where log content will appear once populated.

### [v1.4.23] - 2026-09-07 (Stable)

* Added a discreet, optional PayPal donation link ("Support this project") in the options page footer, with a short note explaining that donations help fund ongoing maintenance and OpenAI API testing. Purely optional — the extension remains fully free and functional without it.

### [v1.4.22] - 2026-09-07 (Stable)

* Fixed the Detected Spam Log and AI Training Memory lists extending the Settings column's height instead of scrolling internally. The Activity column's height is now measured to match the Settings column via JavaScript (kept in sync on window resize and when the resizable Custom Classification Prompt Rules textarea changes size), with each log list capped and independently scrollable within that height.

### [v1.4.21] - 2026-09-07 (Stable)

* Made the two options-page columns match height: the Activity column (Detected Spam Log / AI Training Memory) now stretches to match the Settings column, with its log containers growing to fill the extra vertical space instead of leaving a short box with blank space below it.

### [v1.4.20] - 2026-09-07 (Stable)

* Export backup is now also enabled when Custom Classification Prompt Rules are populated, even with empty spam/training logs, so an imported Conservative Classification Policy can be immediately re-exported as part of a backup.

### [v1.4.19] - 2026-09-07 (Stable)

* Removed the order-radius and semi-transparent white background from the options-page header logo CSS. The rounded corners were revealing the header's teal-green gradient behind the shield icon's transparent corners, creating an unwanted green shadow effect in Pick & Polish and browser preview panes.
...
If you need help using this add-on or encounter a problem, please contact [Product Support](mailto:openai-spam-detector@blastfm.limited). We are happy to help when we can.
